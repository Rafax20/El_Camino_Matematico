...
# KPIs
@onready var kpi_total: Label = $VBoxPrincipal/ContenedorKPIs/TarjetaTotal/Valor
@onready var kpi_precision: Label = $VBoxPrincipal/ContenedorKPIs/TarjetaPrecision/Valor
@onready var kpi_tiempo: Label = $VBoxPrincipal/ContenedorKPIs/TarjetaTiempo/Valor
@onready var kpi_enfoque: Label = $VBoxPrincipal/ContenedorKPIs/TarjetaEnfoque/Valor

# Gráfico y Tabla
@onready var lienzo_grafico: Control = $VBoxPrincipal/ContenedorCuerpo/PanelGrafico/LienzoGrafico
@onready var contenedor_filas_tabla: VBoxContainer = $VBoxPrincipal/ContenedorCuerpo/PanelHistorial/Scroll/VBoxFilas
...
@onready var lbl_diag_icono: Label = $VBoxPrincipal/PanelDiagnostico/VBox/HBoxTit/LabelIcono
...
func _ready():
	...
	if label_maestro:
		label_maestro.text = "👤 Maestro: %s" % (DatosUsuario.nombre_usuario if DatosUsuario.nombre_usuario != "" else "Docente")
...
func _procesar_y_actualizar_ui():
	...
	if kpi_tiempo:
		kpi_tiempo.text = "%.2fs" % tiempo_prom_global
	if kpi_enfoque:
		kpi_enfoque.text = _determinar_categoria_destacada()
		
	# 3. Actualizar Diagnóstico Pedagógico Inteligente
...
func _determinar_categoria_destacada() -> String:
	...
	if peor_cat != "" and peor_pct < 60.0:
		return "⚠️ %s (%.0f%%)" % [peor_cat.capitalize(), peor_pct]
	elif mejor_cat != "":
		return "🌟 %s (%.0f%%)" % [mejor_cat.capitalize(), mejor_pct]
...
func _actualizar_diagnostico_pedagogico(aciertos: int, fallas: int, tiempo_prom: float):
	...
	if lbl_diag_titulo: lbl_diag_titulo.text = diag.get("titulo", "")
	if lbl_diag_icono: lbl_diag_icono.text = diag.get("icono", "ℹ️")
...
# =====================================================================
# 📊 RENDERIZADO DEL GRÁFICO VECTORIAL DE DESEMPEÑO
# =====================================================================
func _dibujar_grafico_desempeno():
	if not lienzo_grafico: return
	var size = lienzo_grafico.size
	...
	var chart_w = size.x - margin_left - margin_right
	var chart_h = size.y - margin_top - margin_bottom
	var chart_bottom = size.y - margin_bottom
	
	# 1. Fondo y Líneas de Cuadrícula Horizontal
	...
	for i in range(5):
		...
		# Línea tenue
		lienzo_grafico.draw_line(Vector2(margin_left, y), Vector2(size.x - margin_right, y), Color(1, 1, 1, 0.08), 1.0)
		# Texto Eje Izquierdo (% Precisión)
	...
	for i in range(4):
		...
		lienzo_grafico.draw_string(font, Vector2(size.x - margin_right + 8, y + 4), "%.0fs" % seg_val, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color("#38bdf8"))
		
	# 2. Dibujar Columnas y Curva por Categoría
	var categorias = ["suma", "resta", "multiplicacion", "division"]
	var nombres_cat = ["SUMA", "RESTA", "MULTI", "DIVISIÓN"]
	...
	# 3. Dibujar Línea y Puntos de Tiempos de Respuesta
...
func _actualizar_tabla_historial():
	...
	for reg in datos_filtrados:
		...
		# Categoría
		var cat_name = str(reg.get("categoria", "Matemáticas")).capitalize()
		...
		lbl_res.custom_minimum_size = Vector2(110, 0)
		...
		var tiempo = float(reg.get("tiempo_tardado", 0.0))
		var lbl_t = Label.new()
		lbl_t.text = "⏱️ %.2f seg" % tiempo
		lbl_t.custom_minimum_size = Vector2(90, 0)
